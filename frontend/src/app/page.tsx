    ...
    <AuthGuard>
      <div className="min-h-screen bg-white flex flex-col">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex-1">
          {/* Combined Inventory Items, Status Legend, and Filters */}
          <div className="bg-white/90 border-l-6 border-[#162C49] border-2 border-black rounded-2xl shadow-xl p-4 sm:p-6 mb-6">
            <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4 mb-2">
              <div>
                <h1 className="text-2xl sm:text-3xl font-bold text-primaryBlue mb-1 tracking-tight">Inventory Items</h1>
                <p className="text-base text-[#162C49] font-medium">Select items you want to borrow</p>
              </div>
              <div className="lg:text-right">
                ...
                <div className="flex flex-wrap gap-3 sm:gap-6">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-700 font-normal">Available</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="w-3 h-3 bg-yellow-200 rounded-full"></span>
                    <span className="text-sm text-gray-700 font-normal">To be Borrowed</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="w-3 h-3 bg-[#162C49] rounded-full"></span>
                    <span className="text-sm text-gray-700 font-normal">Borrowed</span>
                  </div>
                </div>
              </div>
            </div>
            {/* Filters Section */}
            <div>
              <h2 className="text-lg font-semibold text-primaryBlue mb-2">Filters</h2>
              <div className="grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
                {/* Search */}
                <div>
                  <label className="block text-base font-medium text-primaryBlue mb-1">Search</label>
                  <input
                    type="text"
                    placeholder="Search items..."
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#162C49]/60 focus:border-[#162C49] bg-white text-primaryBlue font-medium transition-all duration-300 hover:border-[#162C49]"
                  />
                </div>
                {/* Article Type Filter */}
                <div>
                  <label className="block text-base font-medium text-primaryBlue mb-1">Article Type</label>
                  <select
                ...
                {/* Status Filter */}
                <div>
                  <label className="block text-base font-medium text-primaryBlue mb-1">Status</label>
                  <select
                    value={statusFilter}
                ...
                <div className="flex items-end">
                  <button
                    ...
                    className="w-full px-4 py-2 bg-[#162C49] text-white !important rounded-lg font-semibold shadow-md border border-[#162C49] hover:bg-[#0F1F35] hover:shadow-lg transition-all duration-300 ease-in-out cursor-pointer"
                    style={{ color: 'white' }}
                  >
                    Clear Filters
                  </button>
